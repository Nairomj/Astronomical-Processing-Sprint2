...
namespace Astronomical_Processing
{
    public partial class AstronomicalProcessing : Form
    {
        ...
        private void ButtonSeqSearch_Click(object sender, EventArgs e) 
        {
            ...
            if (!String.IsNullOrEmpty(TextBoxItem.Text))
            {
                ...
                if (target > 9 && target < 100)
                {
                    ...
                    for (int i = 0; i < listData.Length; i++)
                    {
                        if (listData[i] == target && i != dataIndex)
                        {
                            MessageBox.Show("found duplicate data at element [" + i + "]");
                            break;
                        }
                    }
                }
        ...
        private void ButtonMode_Click(object sender, EventArgs e)
        {
            ...
            {
                ModeTextBox.Text = modeTwo.ToString("f") + "\t" + modeOne.ToString("f") + "\t" + modeThree.ToString("f");
            }
            else if (frequencyOne == frequencyTwo && frequencyTwo > frequencyThree && frequencyOne > 1)
            {
                ModeTextBox.Text = modeTwo.ToString("f") + "\t" + modeOne.ToString("f");
            }
            else if (frequencyOne > 1)
                ModeTextBox.Text = modeOne.ToString("f");
            else
        ...
        private void ButtonRange_Click(object sender, EventArgs e)
        {
            ...
            RangeTextBox.Text = range.ToString("f");
        }
        #endregion
    }
}
